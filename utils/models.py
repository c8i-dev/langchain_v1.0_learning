from config.settings import settings
from langchain.chat_models import init_chat_model
from langchain_core.messages import HumanMessage, AIMessage, SystemMessage


model_deepseek = init_chat_model(
    model=settings.model,
    model_provider=settings.model_provider,
    api_key=settings.api_key,
    base_url=settings.base_url,
    temperature=1.0,
    max_tokens=100,
)

model_tongyi = init_chat_model(
    model=settings.tongyi_model,
    model_provider=settings.tongyi_model_provider,
    api_key=settings.tongyi_api_key,
    base_url=settings.tongyi_base_url,
    temperature=1.0,
    max_tokens=1000,
)

model_gemini = init_chat_model(
    model=settings.gemini_model,
    model_provider=settings.gemini_provider,
    api_key=settings.gemini_api_key,
    base_url=settings.gemini_base_url,
    temperature=1.0,
)

model_minimax = init_chat_model(
    model=settings.minimax_model,
    model_provider=settings.minimax_model_provider,
    api_key=settings.minimax_api_key,
    base_url=settings.minimax_base_url,
)

model_openai = init_chat_model(
    model=settings.opneai_model,
    model_provider=settings.opneai_model_provider,
    api_key=settings.opneai_api_key,
    base_url=settings.opneai_base_url,
    temperature=1.0,
    max_tokens=50,
)

model_huanyuan = init_chat_model(
    model=settings.huanyuan_model,
    model_provider=settings.huanyuan_model_provider,
    api_key=settings.huanyuan_api_key,
    base_url=settings.huanyuan_base_url,
)
